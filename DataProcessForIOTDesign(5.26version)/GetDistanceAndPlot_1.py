...
DistanceTrue = [0.5,1,0.5]#三个节点各自的距离实际值

#下面读取文件
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第一组1号0.5m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第一组2号1m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第一组3号0.5m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
#下面开始画图
point = [[0,0],[1.2,0],[-0.12916,0.890679]]#三个点坐标
