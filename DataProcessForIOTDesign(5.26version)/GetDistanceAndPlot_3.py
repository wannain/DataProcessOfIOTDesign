...
DistanceTrue = [2.75,2.95,2.2]#三个节点各自的距离实际值

#下面读取文件
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第三组1号2.75m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第三组2号2.95m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第三组3号2.2m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
#画图
point = [[0,0],[4.25,0],[3.20827,3.69901]]#三个点坐标
