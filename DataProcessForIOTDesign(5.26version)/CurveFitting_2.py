...
Distance = [0.3,0.5,1,2,3,4,6]#距离的种类
Number = [0,0,0,0,0,0,0]#每种距离作为横坐标时需要的数量
...
i = 0
with open('D:/1文件\综合课程设计（物联网）/室外测量拟合结果/节点2号/二号节点测试数据.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
#下面开始剔除每一组的坏值
flag = [0,0,0,0,0,0,0]#等于零的时候说明一遍循环没有提出任何值，可以退出
while(1):
    Std = []#标准差
    Mean = []#平均值
    i = 0
    ...
    while(i != 7):
        for power in Power[i]:
            if (power<Mean[i]-3*Std[i])|(power>Mean[i]+3*Std[i]):
                ...
                flag[i] = 1
        i = i+1
    if(flag[0]|flag[1]|flag[2]|flag[3]|flag[4]|flag[5]|flag[6] == 0):#没有删除任何值
        break
    flag = [0,0,0,0,0,0,0]
#拟合曲线
P_0 = np.mean(Power[2])
DistanceAll = [0.3 for i in range(Number[0])]+[0.5 for i in range(Number[1])]+[1 for i in range(Number[2])]+[2 for i in range(Number[3])]+[3 for i in range(Number[4])]+[4 for i in range(Number[5])]+[6 for i in range(Number[6])]
PowerAll = Power[0]+Power[1]+Power[2]+Power[3]+Power[4]+Power[5]+Power[6]
def func(x,a,b):
