...
DistanceTrue = [4.2,4.75,4.6]#三个节点各自的距离实际值

#下面读取文件
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第四组1号4.2m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
count = 0
with open('D:/1文件\综合课程设计（物联网）/室外三点定位数据/第四组2号4.75m.txt') as f:
    line = f.readline()
    while(1):
        line = f.readline()
        if(len(line.strip())):
...
#画图
point = [[0,0],[3,0],[-4.02,4.04099]]#三个点坐标
